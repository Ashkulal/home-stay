...
export default function AdminContent() {
  ...
  const [homestayForm, setHomestayForm] = useState({
    ...
    location_url: "", check_in_time: "12:00", check_out_time: "11:00", amenities: ""
  });
  ...
  const createPeak = async (e) => {
    ...
    try {
      ...
      loadData();
      toast.success("Peak created successfully!");
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to create peak");
    }
  };
  ...
  const deletePeak = async (id) => {
    ...
    try {
      ...
      loadData();
      toast.success("Peak deleted");
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to delete peak");
    }
  };
  ...
  const deleteHomestay = async (id) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to delete homestay");
    }
  };
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <form onSubmit={createHomestay} className="space-y-3">
              ...
              <input placeholder="Google Maps URL" value={homestayForm.location_url} onChange={(e) => setHomestayForm({ ...homestayForm, location_url: e.target.value })} className="w-full border rounded-lg px-3 py-2" />
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Check-in Time</label>
                  <input type="time" value={homestayForm.check_in_time} onChange={(e) => setHomestayForm({ ...homestayForm, check_in_time: e.target.value })} className="w-full border rounded-lg px-3 py-2" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Check-out Time</label>
                  <input type="time" value={homestayForm.check_out_time} onChange={(e) => setHomestayForm({ ...homestayForm, check_out_time: e.target.value })} className="w-full border rounded-lg px-3 py-2" />
                </div>
              </div>
              <input placeholder="Amenities (comma-separated: WiFi, Parking, etc.)" value={homestayForm.amenities} onChange={(e) => setHomestayForm({ ...homestayForm, amenities: e.target.value })} className="w-full border rounded-lg px-3 py-2" />
              ...
              {homestaysList.map((h) => (
                <div key={h.id} className="flex justify-between items-center border-b pb-2">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">₹{h.price_per_night}/night · {h.max_guests || "?"} guests</p>
                    {h.check_in_time && <p className="text-xs text-gray-400">In: {h.check_in_time} · Out: {h.check_out_time || "11:00"}</p>}
                  </div>
