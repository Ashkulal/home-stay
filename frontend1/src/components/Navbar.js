  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-3" aria-label="Silent Peak Kudremukh Homestay - Home">
          <img src="/logo.png" alt="Silent Peak Kudremukh Homestay Logo" width="48" height="48" className="h-12 w-12 rounded-full object-cover border-2 border-gold-500/40" />
        ...
        <button onClick={() => setOpen(!open)} className="md:hidden text-gold-500 text-2xl p-2" aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open}>
          {open ? "✕" : "☰"}
        </button>

        <div className={`${open ? "flex" : "hidden"} md:flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-6 text-sm absolute md:static top-full left-0 right-0 glass md:bg-transparent md:border-0 p-4 md:p-0`} role="menubar">
          {links.map((l) => (
